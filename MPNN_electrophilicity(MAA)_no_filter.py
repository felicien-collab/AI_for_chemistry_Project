...
def load_site_dataframe(args):
    ...
    n0 = len(df)
    if args.no_maa_filter:
        print(f"  MAA filter disabled — keeping all {n0} rows regardless of MAA value.")
        print(f"  WARNING: dataset may contain DFT outliers or physically nonsensical values.")
    else:
    ...
    print(f"MAA filter active: {not args.no_maa_filter}" + (f" [{args.min_maa}, {args.max_maa}]" if not args.no_maa_filter else " (disabled)"))
...
def main(args):
    ...
    summary = {
        ...
        "maa_filter_active": not args.no_maa_filter,
        "min_maa": args.min_maa if not args.no_maa_filter else "disabled",
        "max_maa": args.max_maa if not args.no_maa_filter else "disabled",
    }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Site-aware GNN v4 — molecule-level split on df_elec.parquet."
    )
    ...
    parser.add_argument(
        "--no_maa_filter", action="store_true",
        help=(
            "Disable the MAA range filter entirely and train on all rows. "
            "By default, rows outside [min_maa, max_maa] are dropped as likely "
            "DFT artifacts. Use this flag to include them and test whether they "
            "hurt or help performance."
        ),
    )
